...
import { UserCheck, Clock, CheckCircle, Calendar, MapPin, Truck } from 'lucide-react'
...
export default function AssignStaffPage() {
  ...
  const items = schedules.map((s) => ({
    ...
    type: s.service?.type ?? '—',
    person: s.reference,
    date: new Date(s.date_time).toLocaleDateString('es-CO', { weekday: 'short', day: 'numeric' }),
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-slate-800">Asignar vehículo</h1>
          <p className="text-sm text-slate-500 mt-0.5">Asigna vehículos a los servicios pendientes</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-5">
        ...
        <StatCard icon={Truck}     iconBg="#DBEAFE" iconColor="#3B82F6" value={inRoute}           label="En ruta" />
        <StatCard icon={CheckCircle} iconBg="#D1FAE5" iconColor="#22C55E" value={completed}      label="Completados" />
        <StatCard icon={UserCheck} iconBg="#F3E8FF" iconColor="#8B5CF6" value={vehicles.length}  label="Vehículos activos" />
      </div>
            ...
            {['all', 'PENDIENTE', 'EN CURSO', 'COMPLETADO'].map((f) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
                  filterStatus === f ? 'text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                }`}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/60">
                    ...
                    <th className="text-left text-xs font-medium text-slate-500 px-5 py-3">Beneficiario</th>
                    ...
                    <th className="text-left text-xs font-medium text-slate-500 px-5 py-3">Acción</th>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map((item) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-3.5 text-sm text-slate-600">{item.person}</td>
                      <td className="px-5 py-3.5">
                      ...
                      <td className="px-5 py-3.5 text-sm text-slate-600">
                        {item.vehicle ?? <span className="text-slate-300 italic text-xs">Sin asignar</span>}
                      </td>
                      <td className="px-5 py-3.5">
        ...
        {selected && selected.status === 'PENDIENTE' && (
          <div className="w-72 flex-shrink-0">
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-5">
              <h3 className="font-semibold text-slate-700 mb-4">Asignar vehículo</h3>
              ...
              <div className="mb-4">
                ...
                {vehicles.length === 0 ? (
                  <p className="text-xs text-slate-400 italic">No hay vehículos registrados</p>
                ) : (
                    ...
                    {vehicles.map((v) => (
                      ...
                      >
                        <input
                          ...
                          onChange={() => setVehicleAssign((p) => ({ ...p, [selected.id]: v.id }))}
                          className="accent-blue-600"
                        />
                        <div className="flex-1">
                          <p className="font-medium text-slate-800 text-sm">{v.name}</p>
                          <p className="text-xs text-slate-400">{v.license_plate} · {v.capacity ?? '—'} pasajeros</p>
                        </div>
                      </label>
                    ))}
                  </div>
              ...
              <button
                ...
                onClick={() => {
                  if (vehicleAssign[selected.id])
                    assignMut.mutate({ id: selected.id, vehicle_id: vehicleAssign[selected.id] })
                }}
              >
