...
const schema = z.object({
  ...
  price:       z.string().regex(/^\d+(\.\d{1,2})?$/, 'Formato inválido (ej. 50000 o 50000.00)'),
...
function validateFile(file: File): string | null {
  if (!ACCEPTED.includes(file.type)) return 'Solo se permiten imágenes JPEG o PNG'
...
export default function ProductsPage() {
  ...
  const { data: products   = [], isLoading, isError, error: productsError } = useQuery({ queryKey: ['products'],   queryFn: getProducts })
  ...
  const openCreate = () => {
    setEditTarget(null)
    resetFile()
    reset({ id_category: '', name: '', description: '', price: '', stock: 0, active: true })
    setModalOpen(true)
  }
  ...
  // Miniatura con fallback cuando la imagen falla
  const ProductThumbnail = ({ photo, name }: { photo: string; name: string }) => {
    const [imgError, setImgError] = useState(false)
    return (
      <div className="w-12 h-12 rounded-lg overflow-hidden flex-shrink-0 border border-slate-100 bg-slate-50 flex items-center justify-center">
        {photo && !imgError ? (
          <img
            src={photo}
            alt={name}
  ...
  // ── Zona de imagen (reutilizable en el modal) ─────────────────
  ...
  const ImageZone = () => (
    <div>
      <label className="block text-sm font-medium text-slate-700 mb-1.5">
        Imagen del producto{!editTarget && <span className="text-red-500"> *</span>}
      </label>

      {displayUrl ? (
        // Preview con overlay "Cambiar"
        <div className="relative group rounded-xl overflow-hidden border border-slate-200 bg-slate-50 h-44">
          ...
          <label className="absolute inset-0 flex flex-col items-center justify-center gap-1.5 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
            ...
            <span className="text-white/60 text-[11px]">JPEG o PNG · máx. 5 MB</span>
            <input
              ref={fileInputRef}
              type="file"
        ...
        >
          <ImagePlus size={28} className={fileError ? 'text-red-300' : 'text-slate-300'} />
          <p className="mt-2 text-sm font-medium text-slate-400">Haz clic para subir imagen</p>
          <p className="text-xs text-slate-300 mt-0.5">JPEG o PNG · máx. 5 MB</p>
          <input
            ref={fileInputRef}
            type="file"
            accept="image/jpeg,image/jpg,image/png"
      ...
      {fileError && (
        <p className="text-red-500 text-xs mt-1.5 flex items-center gap-1">{fileError}</p>
      )}
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-slate-800">Catálogo de productos</h1>
          <p className="text-sm text-slate-500 mt-0.5">
            {displayedProducts.length} de {products.length} productos
          </p>
        </div>
        <Button onClick={openCreate}><Plus size={14} /> Nuevo producto</Button>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        {isLoading ? (
          <div className="p-10 text-center text-slate-400 text-sm">Cargando productos...</div>
        ) : isError ? (
          <div className="p-10 text-center">
            <p className="text-red-500 text-sm font-medium">Error al cargar los productos</p>
            <p className="text-slate-400 text-xs mt-1">
              {(productsError as any)?.response?.data?.message ?? (productsError as any)?.message ?? 'Verifica la conexión con el servidor'}
            </p>
          </div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/60">
                <th className="text-left text-xs font-medium text-slate-500 px-5 py-3 w-[40%]">Producto</th>
                <th className="text-left text-xs font-medium text-slate-500 px-5 py-3">Categoría</th>
                ...
                <tr key={p.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        {p.description && (
                          <p className="text-xs text-slate-400 truncate max-w-[160px]">{p.description}</p>
                        )}
                      </div>
                    </div>
                  </td>

                  <td className="px-5 py-3 text-sm text-slate-500">{catName(p.id_category)}</td>
                  ...
                  <td className="px-5 py-3">
                    <span className={`text-sm font-semibold ${stockColor(p.stock)}`}>
                      {p.stock}
                      {p.stock === 0 && <span className="ml-1 text-xs font-normal">(agotado)</span>}
                    </span>
                  </td>
              ...
              {displayedProducts.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-5 py-12 text-center text-slate-400 text-sm">
                    {filterCat ? 'No hay productos en esta categoría.' : 'No hay productos registrados.'}
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <Modal
        open={modalOpen}
        onClose={() => { if (!isPending) closeModal() }}
          ...
          <div>
            ...
            <select
              className={`w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 bg-white ${
                errors.id_category ? 'border-red-400 bg-red-50' : 'border-slate-200'
              }`}
            ...
            {errors.id_category && (
              <p className="text-red-500 text-xs mt-1">{errors.id_category.message}</p>
            )}
          </div>
          ...
          {editTarget && (
            <div className="flex items-center justify-between rounded-lg border border-slate-200 px-4 py-3">
              <div>
                ...
                <p className="text-xs text-slate-400">Visible y disponible para compra en la app</p>
              </div>
              <button
                type="button"
