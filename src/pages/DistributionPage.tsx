...
export default function DistributionPage() {
  ...
  const unassigned = schedules.filter(
    (s) => !s.status || s.status === "PENDIENTE",
  );
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Asigna y distribuye servicios a los usuarios registrados
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-5">
        ...
        <div className="flex-1 bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
          ...
          <div className="grid grid-cols-5 min-h-64">
            {isLoading ? (
              <div className="col-span-5 py-12 text-center text-slate-400 text-sm">
                Cargando...
              </div>
            ) : (
              DAYS.map((_, i) => {
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div className="p-3 space-y-3">
              ...
              {unassigned.slice(0, 5).map((s) => {
                const style =
                  TYPE_STYLE[s.service?.type ?? "default"] ??
                  TYPE_STYLE.default;
                return (
                  <div key={s.id} className="bg-slate-50 rounded-lg p-3">
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-slate-400">
                      <span className="flex items-center gap-1">
                        <User size={10} /> {s.reference}
                      </span>
                      <span className="flex items-center gap-1">
                        <MapPin size={10} />
                        {new Date(s.date_time).toLocaleDateString("es-CO", {
                          weekday: "short",
                          day: "numeric",
                          month: "short",
                        })}
