...
import { Clock, Truck, CheckCircle, Phone, User, ChevronDown } from 'lucide-react'
import { getSchedulesAcompan, updateScheduleAcompan, type ScheduleStatus } from '@/api/schedules'
import Badge from '@/components/ui/Badge'
import StatCard from '@/components/ui/StatCard'

const STATUS_CONFIG: Record<ScheduleStatus, { label: string; variant: 'warning' | 'info' | 'success'; icon: React.ElementType; dot: string }> = {
  'PENDIENTE':  { label: 'Pendiente',   variant: 'warning', icon: Clock,        dot: '#F59E0B' },
  'EN CURSO':   { label: 'En ruta',     variant: 'info',    icon: Truck,         dot: '#3B82F6' },
  'COMPLETADO': { label: 'Completado',  variant: 'success', icon: CheckCircle,   dot: '#22C55E' },
}
...
export default function ServiceStatusPage() {
  ...
  const updateMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['schedules-acompan'] }); setUpdatingId(null) },
  })
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-slate-800">Estado del servicio</h1>
          <p className="text-sm text-slate-500 mt-0.5">Monitorea el estado en tiempo real de los servicios activos</p>
        </div>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-3 gap-4 mb-5">
        <StatCard icon={Clock}       iconBg="#FEF3C7" iconColor="#F59E0B" value={counts['PENDIENTE']}  label="Pendientes" />
        <StatCard icon={Truck}       iconBg="#DBEAFE" iconColor="#3B82F6" value={counts['EN CURSO']}   label="En ruta" />
        <StatCard icon={CheckCircle} iconBg="#D1FAE5" iconColor="#22C55E" value={counts['COMPLETADO']} label="Completados" />
      </div>
        ...
        {(['all', 'PENDIENTE', 'EN CURSO', 'COMPLETADO'] as const).map((f) => {
          ...
          return (
            <button
              ...
              style={filter === f ? { backgroundColor: cfg?.dot ?? '#1D3461' } : {}}
            >
      ...
      {isLoading ? (
        <div className="py-12 text-center text-slate-400 text-sm">Cargando servicios...</div>
      ) : (
        /* Cards grid */
              ...
              <div key={item.id} className="bg-white rounded-xl shadow-sm border border-slate-100 p-4">
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <span className="inline-block text-xs text-slate-400 mt-0.5">{item.type}</span>
                  </div>
                ...
                <div className="flex items-center gap-4 mb-3 text-xs text-slate-500">
                  <span className="flex items-center gap-1"><User size={11} /> {item.person}</span>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-50">
                  <div className="text-xs text-slate-400">
                    {item.vehicle
                      ? <span className="flex items-center gap-1"><Truck size={10} /> {item.vehicle}</span>
                  ...
                  <div className="mt-3">
                    {updatingId === item.id ? (
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => setUpdatingId(null)} className="px-2 py-1 text-xs text-slate-400">✕</button>
                      </div>
                    ) : (
                      <button
