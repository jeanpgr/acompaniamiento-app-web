...
import { z } from 'zod'
import {
  getDetailsDaycare, createDetailDaycare, updateDetailDaycare, deleteDetailDaycare,
  type DetailDaycare, type CreateDetailDaycareInput,
...
const schema = z.object({
  ...
  mode_hours:    z.coerce.number().int().nonnegative().optional(),
...
export default function DaycareDetailPage() {
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<CreateDetailDaycareInput> }) =>
  ...
  const modeName = (item: DetailDaycare) =>
    item.service_mode?.name ?? <span className="text-slate-300 italic text-xs">—</span>

  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-slate-800">Detalles de guardería</h1>
          <p className="text-sm text-slate-500 mt-0.5">Gestiona planes y modalidades del servicio de guardería</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        {isLoading ? (
          <div className="p-8 text-center text-slate-400 text-sm">Cargando planes...</div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/60">
                <th className="text-left text-xs font-medium text-slate-500 px-5 py-3">Modalidad</th>
                <th className="text-left text-xs font-medium text-slate-500 px-5 py-3">Horas</th>
                <th className="text-left text-xs font-medium text-slate-500 px-5 py-3">
                  <div className="flex items-center gap-1"><MapPin size={12} /> Punto de atención</div>
                </th>
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                  <td className="px-5 py-3.5 font-medium text-slate-800 text-sm">{modeName(item)}</td>
                  ...
                  <td className="px-5 py-3.5 text-sm text-slate-500 max-w-[180px] truncate">
                    {item.address_point ?? <span className="text-slate-300 italic text-xs">—</span>}
                  </td>
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (window.confirm('¿Eliminar este plan de guardería?')) deleteMut.mutate(item.id)
                        }}
                      >
      ...
      >
        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Nombre de modalidad</label>
              <input
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Punto de atención</label>
            <input
              className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none"
