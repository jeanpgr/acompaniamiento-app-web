...
import { z } from 'zod'
import {
  getDetailsTraining, createDetailTraining, updateDetailTraining, deleteDetailTraining,
  type DetailTraining, type CreateDetailTrainingInput,
...
const schema = z.object({
  ...
  duration:    z.coerce.number().int().positive('Debe ser un número positivo'),
...
export default function TrainingDetailPage() {
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<CreateDetailTrainingInput> }) =>
  ...
  const openCreate = () => {
    setEditTarget(null)
    reset({ id_service: '', topic: '', description: '', date_time: '', duration: 60, link_meet: '' })
    setModalOpen(true)
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-slate-800">Detalles de capacitación</h1>
          <p className="text-sm text-slate-500 mt-0.5">Gestiona talleres y cursos de formación</p>
        </div>
            ...
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/60">
                ...
                <th className="text-left text-xs font-medium text-slate-500 px-5 py-3">
                  <div className="flex items-center gap-1"><Clock size={12} /> Duración (min)</div>
                </th>
                <th className="text-left text-xs font-medium text-slate-500 px-5 py-3">
                  <div className="flex items-center gap-1"><Video size={12} /> Enlace</div>
                </th>
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                  ...
                  <td className="px-5 py-3.5">
                    {item.link_meet ? (
                      <a href={item.link_meet} target="_blank" rel="noopener noreferrer"
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (window.confirm(`¿Eliminar "${item.topic}"?`)) deleteMut.mutate(item.id)
                        }}
                      >
        ...
        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          ...
          <div>
            ...
            <input
              ...
              {...register('link_meet')}
            />
            {errors.link_meet && <p className="text-red-500 text-xs mt-1">{errors.link_meet.message}</p>}
          </div>
        </form>
      </Modal>
    </div>
