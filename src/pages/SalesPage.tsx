...
const STATUS_CFG: Record<SalesStatus, { label: string; bg: string; color: string }> = {
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-0.5">Historial de ventas y gestión de estados</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        {isLoading ? (
          <div className="p-8 text-center text-slate-400 text-sm">Cargando ventas...</div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/60">
                <th className="text-left text-xs font-medium text-slate-500 px-5 py-3">ID</th>
                <th className="text-left text-xs font-medium text-slate-500 px-5 py-3">Usuario</th>
                <th className="text-left text-xs font-medium text-slate-500 px-5 py-3">Subtotal</th>
                <th className="text-left text-xs font-medium text-slate-500 px-5 py-3">Descuento</th>
                <th className="text-left text-xs font-medium text-slate-500 px-5 py-3">Total</th>
                <th className="text-left text-xs font-medium text-slate-500 px-5 py-3">Estado</th>
                <th className="text-left text-xs font-medium text-slate-500 px-5 py-3">Fecha</th>
            ...
            <tbody>
              {sales.map((sale) => (
                <tr key={sale.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                  ...
                  <td className="px-5 py-3.5">
                    <span className="font-mono text-xs text-slate-500">{sale.id_user.slice(0, 8)}…</span>
                  </td>
                  <td className="px-5 py-3.5 text-sm text-slate-600">$ {sale.subtotal}</td>
                  <td className="px-5 py-3.5 text-sm text-emerald-600">- $ {sale.discount}</td>
                  <td className="px-5 py-3.5 text-sm font-semibold text-slate-800">$ {sale.total}</td>
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex gap-2">
                      <Button size="sm" onClick={() => setDetailSaleId(sale.id)}>
                        <Eye size={12} /> Detalle
                      </Button>
                      <Button size="sm" onClick={() => { setStatusModal(sale); setNewStatus(sale.status ?? 'PENDING') }}>
      ...
      >
        {selectedSale && (
          <div className="flex gap-4 mb-4 pb-4 border-b border-slate-100">
            <div className="flex-1 text-center">
              ...
              <p className="text-sm font-medium text-slate-700">$ {selectedSale.subtotal}</p>
            </div>
            <div className="flex-1 text-center">
              <p className="text-xs text-slate-400">Descuento</p>
              <p className="text-sm font-medium text-emerald-600">- $ {selectedSale.discount}</p>
            </div>
            <div className="flex-1 text-center">
              <p className="text-xs text-slate-400">Total</p>
              <p className="text-sm font-bold text-slate-800">$ {selectedSale.total}</p>
            </div>
            <div className="flex-1 text-center">
        ...
        {detailsLoading ? (
          <p className="text-sm text-slate-400 text-center py-4">Cargando detalle...</p>
        ) : details.length === 0 ? (
          <p className="text-sm text-slate-400 text-center py-4">Sin ítems en esta venta</p>
        ) : (
            ...
            <tbody>
              {details.map((d) => (
                <tr key={d.id} className="border-b border-slate-50">
                  <td className="py-2 text-slate-700 font-medium">{productName(d.id_product)}</td>
                  <td className="py-2 text-right text-slate-600">{d.quantity}</td>
                  <td className="py-2 text-right text-slate-600">$ {d.unit_price}</td>
                  <td className="py-2 text-right font-medium text-slate-800">$ {d.subtotal}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        footer={
          <>
            ...
            <Button
              ...
              onClick={() => statusModal && updateMut.mutate({ id: statusModal.id, status: newStatus })}
            >
              Guardar
            </Button>
          </>
      ...
      >
        <div className="space-y-3">
          {(['PENDING', 'COMPLETED', 'CANCELLED'] as SalesStatus[]).map((s) => (
            <label key={s} className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${newStatus === s ? 'border-blue-400 bg-blue-50' : 'border-slate-200 hover:bg-slate-50'}`}>
