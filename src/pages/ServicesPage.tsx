...
const schema = z.object({
  ...
  price:       z.string().regex(/^\d+(\.\d{1,2})?$/, 'Formato inválido (ej. 50 o 50.00)').optional().or(z.literal('')),
})
...
function TypeBadge({ type }: { type: ServiceType | null }) {
  ...
  const cfg = TYPE_COLORS[type] ?? { bg: '#F1F5F9', text: '#475569', label: type }
  return (
    <span
...
export default function ServicesPage() {
  ...
  const createMut = useMutation({
    ...
    onError: (err: any) => {
      const msg = err?.response?.data?.message ?? err?.message ?? 'Error al crear el servicio'
      toast.error(msg)
    },
  })
  ...
  const updateMut = useMutation({
    ...
    onError: (err: any) => {
      const msg = err?.response?.data?.message ?? err?.message ?? 'Error al actualizar el servicio'
      toast.error(msg)
    },
  })
  ...
  const deleteMut = useMutation({
    ...
    onError: (err: any) => {
      const msg = err?.response?.data?.message ?? err?.message ?? 'Error al eliminar el servicio'
      toast.error(msg)
    },
  })
  ...
  const onSubmit = (data: FormData) => {
    if (editTarget) {
      ...
      createMut.mutate(sanitizePayload(data) as any)
    }
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Administra el catálogo de servicios y su disponibilidad</p>
        </div>
      ...
      <div className="flex items-center gap-1 mb-4 border-b border-slate-200">
        {([
          ...
          <button
            ...
            style={tab === key && key === 'all' ? { backgroundColor: '#1D3461', borderColor: '#1D3461' } : {}}
          >
            {label}
          </button>
        ))}
              ...
              {displayed.map((s) => (
                <tr key={s.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                  <td className="px-5 py-3.5">
                    ...
                    {s.description && (
                      <p className="text-xs text-slate-400 truncate max-w-52">{s.description}</p>
                    )}
                  </td>
                  <td className="px-5 py-3.5"><TypeBadge type={s.type} /></td>
                  <td className="px-5 py-3.5 text-sm text-slate-500">
                    {s.price ? `$ ${s.price}` : <span className="text-slate-300 italic text-xs">—</span>}
                  </td>
                  <td className="px-5 py-3.5 text-sm text-slate-500">
...
function sanitizePayload(data: Partial<FormData>) {
  const out: Record<string, any> = {}
  if (data.name)        out.name        = data.name.trim()
  if (data.description) out.description = data.description.trim()
  // Empty string means "no type selected" → omit
  if (data.type && data.type !== ('' as any)) out.type = data.type
  // Empty or blank price → omit; backend expects "50.00" string format
