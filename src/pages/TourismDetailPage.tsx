...
import { z } from 'zod'
import {
  getDetailsTourism, createDetailTourism, updateDetailTourism, deleteDetailTourism,
  type DetailTourism, type CreateDetailTourismInput,
...
const schema = z.object({
  ...
  quotas:                 z.coerce.number().int().positive('Debe ser un número positivo'),
...
export default function TourismDetailPage() {
  ...
  const openCreate = () => {
    setEditTarget(null)
    reset({ id_service: '', name: '', description: '', date_output: '', date_arrival: '', quotas: 1, meeting_point_address: '' })
    setModalOpen(true)
  }
  ...
  const onSubmit = (data: FormData) => {
    const payload: CreateDetailTourismInput = {
      ...
      ...(data.meeting_point_address && { meeting_point_address: data.meeting_point_address }),
    }
    if (editTarget) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-slate-800">Detalles de turismo</h1>
          <p className="text-sm text-slate-500 mt-0.5">Gestiona excursiones y salidas turísticas</p>
        </div>
            ...
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/60">
                ...
                <th className="text-left text-xs font-medium text-slate-500 px-5 py-3">
                  <div className="flex items-center gap-1"><Users size={12} /> Cupos</div>
                </th>
                <th className="text-left text-xs font-medium text-slate-500 px-5 py-3">
                  <div className="flex items-center gap-1"><MapPin size={12} /> Punto encuentro</div>
                </th>
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                  ...
                  <td className="px-5 py-3.5 text-sm text-slate-500 max-w-[160px] truncate">
                    {item.meeting_point_address ?? <span className="text-slate-300 italic text-xs">—</span>}
                  </td>
                  <td className="px-5 py-3.5">
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (window.confirm(`¿Eliminar "${item.name}"?`)) deleteMut.mutate(item.id)
                        }}
                      >
      ...
      >
        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
          <div>
            ...
            {errors.id_service && <p className="text-red-500 text-xs mt-1">{errors.id_service.message}</p>}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              />
              {errors.date_output && <p className="text-red-500 text-xs mt-1">{errors.date_output.message}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">
              ...
              />
              {errors.date_arrival && <p className="text-red-500 text-xs mt-1">{errors.date_arrival.message}</p>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                {...register('quotas')}
              />
              {errors.quotas && <p className="text-red-500 text-xs mt-1">{errors.quotas.message}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Punto de encuentro</label>
              <input
