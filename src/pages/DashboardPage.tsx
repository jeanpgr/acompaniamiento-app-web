...
function ReviewsChart({ reviews }: { reviews: { grade: number; created_at: string }[] }) {
...
export default function DashboardPage() {
  ...
  const { data: schedules = [] } = useQuery({ queryKey: ['schedules-acompan'], queryFn: getSchedulesAcompan })
  ...
  const typeCount = (type: string) => services.filter((s) => s.type === type).length

  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-slate-800">Métricas y satisfacción</h1>
          <p className="text-sm text-slate-500 mt-0.5">Analiza resultados, tendencias y encuestas de satisfacción</p>
        </div>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-4 gap-4 mb-6">
        <StatCard icon={Briefcase} iconBg="#DBEAFE" iconColor="#1D4ED8" value={services.filter(s => s.active).length} label="Servicios activos" />
        <StatCard icon={Clock}     iconBg="#FEF3C7" iconColor="#F59E0B" value={pending}   label="Pendientes" />
        <StatCard icon={Truck}     iconBg="#D1FAE5" iconColor="#22C55E" value={inRoute}   label="En ruta" />
        <StatCard icon={Star}      iconBg="#FEF9C3" iconColor="#D97706" value={avgGrade}  label="Calificación promedio" />
      </div>

      <div className="grid grid-cols-3 gap-5 mb-6">
        {/* Calificación semanal */}
        <div className="col-span-2 bg-white rounded-xl shadow-sm border border-slate-100 p-5">
          <h2 className="text-sm font-semibold text-slate-700 mb-4">Calificación semanal promedio</h2>
          <ReviewsChart reviews={reviews} />
        </div>

        {/* Servicios por tipo */}
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-5">
          <h2 className="text-sm font-semibold text-slate-700 mb-4">Servicios por tipo</h2>
          <div className="space-y-3">
            {[
              { key: 'ACOMPAÑAMIENTO', label: 'Acompañamiento', color: '#3B82F6' },
            ...
            {services.length === 0 && (
              <p className="text-xs text-slate-400 text-center py-4">Sin servicios registrados</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-5">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-5">
          <h2 className="text-sm font-semibold text-slate-700 mb-4">Distribución de calificaciones</h2>
        ...
        {/* Reseñas recientes */}
        <div className="col-span-2 bg-white rounded-xl shadow-sm border border-slate-100 p-5">
          <h2 className="text-sm font-semibold text-slate-700 mb-4">Reseñas recientes</h2>
          {recentReviews.length === 0 ? (
            <p className="text-sm text-slate-400 text-center py-6">Sin reseñas registradas aún</p>
          ) : (
            <div className="space-y-3">
              {recentReviews.map((r) => (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="text-xs font-semibold text-slate-700">
                        {'★'.repeat(r.grade)}{'☆'.repeat(5 - r.grade)}
                      </span>
                      <span className="text-xs text-slate-400">
                        {new Date(r.created_at).toLocaleDateString('es-CO')}
                      </span>
                    </div>
                    <p className="text-xs text-slate-600 truncate">{r.comment}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {schedules.length > 0 && (
        <div className="mt-5 bg-white rounded-xl shadow-sm border border-slate-100 p-5">
          <h2 className="text-sm font-semibold text-slate-700 mb-4">Resumen de agendamientos (Acompañamiento)</h2>
          <div className="grid grid-cols-3 gap-4 text-center">
            {[
              ...
              <div key={label} className="rounded-lg p-4" style={{ backgroundColor: color + '18' }}>
